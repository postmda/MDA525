...
dotPlot(x, xlim = c(0.9 * min(x), 1.05 * max(x)), ylim = c(0.96, 1.05), xlab = "Interest rate", ylab = "", at = 1.0007, pch = 20, col = COL[1], cex = 2, axes = F)
abline(h = 0.98)
...
* Intensity Maps
  ...
  - Colors are used to show higher and lower values.
  - Helpful for showing geographic trends.
...
# 2015 State Population Intensity Map
plot_usmap(data=statepop, values="pop_2015", color="yellow")+scale_fill_continuous(name="Population (2015)", low="white", high="green", label=scales::comma)+theme(legend.position="right")
```
...
* Standardized version of bar plots
